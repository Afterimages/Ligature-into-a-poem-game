...
严格保证只使用上下左右四个方向，不会出现斜向连接。当预期位置不可用时，会智能选择替代方向，保持排列的连续性
...
- 5）
  - 1.每一关都设置时间和步数限制
  - 2.设置提示，（遇到不会连的诗句，可以通过提示获得，提示需要消耗某一物品，这个物品可以通过前一关通关获得，第一关可以初始赠予）。
  - 3.诗句合理布局，形成分布一个饱满的形状（诗句数量和难度随着关卡提升，分布形成的形状同步扩大）。
  - 4.视听觉体验上，诗句在连线时字框的颜色可以更加明显；还有文字的字体也可以改成楷体或者宋体之类的；游戏界面的背景可以设置成符合游戏的图片；同上可以播放合适的背景音乐（游戏玩家可以选择开启或关闭）。
  - 5.游戏体验，每一句诗连成可以震动以示成功（震动也可以自主选择开启或关闭），可以增加小特效以示成功和鼓励。
- 6）下个版本可以搞难一点的不
- 7）格子太小了，不好点击，设置字体大小。
- 8）添加主菜单、分数历史、排行榜